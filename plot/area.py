...
# triangulation of instance points  -------------------------------------------
    ...
    for i,sim in enumerate(t_tri.simplices):
        ...
        for i in range(len(sim)): # insertion by longest distance
            ...
            etemp = copy.copy(e[0])
            e[0] = max(e[0], dist)
            if etemp != e[0]: e[3] = instance_points[sim[i]]
...
# write to json file ----------------------------------------------------------
    ...
    if arguments.plot: 
        ...
        for i,p in enumerate(t_midpoints):
            ax.plot(p[1],p[2],"rP")
            #"""
            ax.plot((p[1],p[3][0]),(p[2],p[3][1]),"r-")
            """
            triangle = Polygon((instance_points[t_midpoints[i][3][0]],instance_points[t_midpoints[i][3][1]],instance_points[t_midpoints[i][3][2]]), fc=(1,1,1,0), ec=(1,0,0,1), lw=2)
